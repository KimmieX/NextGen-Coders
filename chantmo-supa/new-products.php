...
// Handle newsletter subscription FIRST
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['subscribe_email'])) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $body = "Thank you for subscribing to our new product notifications!";
        
        if (sendEmail($email, $subject, $body)) {
            $_SESSION['message'] = "Subscription successful! Check your email for confirmation.";
            redirect($_SERVER['REQUEST_URI']);
        } else {
            $_SESSION['message'] = "Failed to send confirmation email. Please try again later.";
        }
    } else {
...
try {
    $newProducts = Product::getNewArrivals($perPage, $offset, $sortField, $sortOrder);
    $totalProducts = Product::count(['badge' => 'New']);
    ...
    $totalProducts = 0;
    $totalPages = 1;
    echo '<div class="alert alert-danger">Error loading products. Please try again later.</div>';
}

// Get recently viewed products with enhanced handling
$recentlyViewed = [];
    ...
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-6">
                ...
                <p class="lead mb-4">Discover our latest products added this week.</p>
                <div class="d-flex gap-3">
                    <a href="<?= base_url('deals.php') ?>" class="btn btn-primary btn-lg px-4">
                        <i class="fas fa-tag me-2"></i> Special Offers
                    </a>
                    <a href="<?= base_url('categories.php') ?>" class="btn btn-outline-primary btn-lg px-4">
                        <i class="fas fa-list me-2"></i> Browse Categories
                    </a>
                </div>
            </div>
            <div class="col-lg-6">
                <img src="https://media.istockphoto.com/id/1429152318/photo/students-move-into-a-dorm-or-student-accommodation.webp?a=1&b=1&s=612x612&w=0&k=20&c=lYOoPyxWlnXR4vi9T33WTIRMtL7ziAeZeBUYVxgPwfI=" 
                     alt="New Products" class="img-fluid rounded shadow-lg">
            </div>
        </div>
    </div>
</section>
...
<section class="py-5">
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <div>
                <h2 class="fw-bold mb-2">Our Newest Products</h2>
                <p class="text-muted">
        ...
        <?php else: ?>
            ...
            <?php foreach (array_chunk($newProducts, 4) as $productGroup): ?>
...
<section class="newsletter-section bg-primary text-white py-5">
    <div class="container text-center">
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <h2 class="fw-bold mb-3">Want New Arrival Alerts?</h2>
                <p class="lead mb-4">Subscribe to be the first to know about our latest products.</p>
                
                <form class="newsletter-form" id="newProductsNewsletterForm" action="<?= base_url('includes/newsletter-subscribe.php') ?>" method="POST">
                    ...
                    <div class="form-text mt-2 text-white-50">
                        We'll email you when new products arrive. Unsubscribe anytime.
                    </div>
                </form>
            </div>
        </div>
...
<script>
// Newsletter Form Handling for new products page
document.addEventListener('DOMContentLoaded', function() {
    const newsletterForm = document.getElementById('newProductsNewsletterForm');
