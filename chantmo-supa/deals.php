...
$sortOptions = [
    ...
    'discount' => ['field' => '(original_price - price)', 'order' => 'DESC'] // New discount sorting option
];
...
// Get discounted products with error handling
try {
    $discountedProducts = Product::getDiscountedProducts($perPage, $offset, $sortField, $sortOrder);
    $totalProducts = Product::count(['conditions' => ['original_price > price']]);
    $totalPages = ceil($totalProducts / $perPage);
} catch (Exception $e) {
    error_log("Error loading discounted products: " . $e->getMessage());
    $discountedProducts = [];
    $totalProducts = 0;
    $totalPages = 1;
    echo '<div class="alert alert-danger">Error loading discounted products. Please try again later.</div>';
}
    ...
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-6">
                ...
                <p class="lead mb-4">Discover our discounted products and save money today!</p>
                <div class="d-flex gap-3">
                    <a href="<?= base_url('new-products.php') ?>" class="btn btn-primary btn-lg px-4">
                        <i class="fas fa-star me-2"></i> New Arrivals
                    </a>
                    <a href="<?= base_url('featured-products.php') ?>" class="btn btn-outline-primary btn-lg px-4">
                        <i class="fas fa-gem me-2"></i> Featured Products
                    </a>
                </div>
            </div>
            <div class="col-lg-6">
                <img src="https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1470&q=80" 
                     alt="Special Offers" class="img-fluid rounded shadow-lg">
            </div>
        </div>
    </div>
</section>
...
<section class="py-5">
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <div>
                <h2 class="fw-bold mb-2">Current Deals</h2>
                <p class="text-muted">
                    <?= $totalProducts ?> discounted products
                    <?php if ($totalPages > 1): ?>
        ...
        <?php else: ?>
            ...
            <?php foreach (array_chunk($discountedProducts, 4) as $productGroup): ?>
...
<section class="newsletter-section bg-primary text-white py-5">
    <div class="container text-center">
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <h2 class="fw-bold mb-3">Want Deal Alerts?</h2>
                <p class="lead mb-4">Subscribe to get updates on our latest discounts and special offers.</p>
                
                <form class="newsletter-form" id="dealsNewsletterForm" action="<?= base_url('includes/newsletter-subscribe.php') ?>" method="POST">
                    ...
                    <div class="form-text mt-2 text-white-50">
                        We'll email you about new deals. Unsubscribe anytime.
                    </div>
                </form>
            </div>
        </div>
...
<script>
// Newsletter Form Handling for deals page
document.addEventListener('DOMContentLoaded', function() {
    const newsletterForm = document.getElementById('dealsNewsletterForm');
